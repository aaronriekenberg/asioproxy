...
private:
	...
	TcpProxySession(boost::asio::io_service& ioService);
	...
	void handleRemoteEndpointResolved(const boost::system::error_code& error,
	...
	void handleRemoteRead(const boost::system::error_code& error,
			size_t bytesRead);

	void handleRemoteWriteFinished(const boost::system::error_code& error);
