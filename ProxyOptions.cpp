...
const ProxyOptions& ProxyOptions::createInstance(
  int argc, char** argv)
{
  if (m_pInstance)
  {
...
ProxyOptions::ProxyOptions(int argc, char** argv)
{
  ...
  desc.add_options()
    ...
    ("remote-address,r",
     ...
     "remote address")
    ("buffer-size,b",
     boost::program_options::value<size_t>(&m_bufferSize)->
       default_value(DEFAULT_BUFFER_SIZE),
     "buffer size (bytes)")
    ("connect-timeout,c",
     boost::program_options::value<long>(&connectTimeoutMS)->
       default_value(DEFAULT_CONNECT_TIMEOUT_MS),
     "connect timeout (milliseconds)")
    ("num-threads,t",
     boost::program_options::value<size_t>(&m_numThreads)->
  ...
  std::vector<std::string> localAddresses =
    vm["local-address"].as<std::vector<std::string> >();
  m_localAddressPortVector.reserve(localAddresses.size());
  std::transform(localAddresses.begin(),
                 localAddresses.end(),
  ...
  m_remoteAddressPort =
    parseAddressPortString(vm["remote-address"].as<std::string>());
...
ProxyOptions::AddressAndPort
ProxyOptions::parseAddressPortString(const std::string& addressPortString)
{
  bool error = false;
  const std::string::size_type colonPos =
    addressPortString.find_last_of(':');
